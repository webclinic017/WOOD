...
'''
In this Version, the most significative change is the static RL, and several minor changes as save bt
'''
...
print(col.Fore.YELLOW,col.Back.BLUE,'--- Golem FX #v1.4 ---',col.Style.RESET_ALL)
print('')
...
def guess_closeS(df1): 
    ...
    df1_final = df1[['Open','High','Low','Close']].shift(1)
...
def importohlc(df,df1,_suffix):
    df['Date'] = pd.to_datetime(df.Date)
    df1['Date'] = pd.to_datetime(df1.Date)
    df = df.join(df1[['OpenShift','HighShift','LowShift','CloseShift']],how='left',on='Date',rsuffix=_suffix)
    df.rename(columns = {'OpenShift':'OpenShift'+_suffix,'HighShift':'HighShift'+_suffix,'LowShift':'LowShift'+_suffix,'CloseShift':'CloseShift'+_suffix},inplace=True)
...
def bt(price,_year_bottom,_year_top,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_invers,_trigger_sl,_verbose,_cash_ini,\
        _rate,x,_target,_exposure,_size,_sl,_save=0):
    engine = pyttsx3.init()
...
def make_indicators(df, df_H1):
    ...
    df = importohlc(df,df_H1,_suffix='_hourly')
...
# Conexion a FXCM
    ...
    while True:
        ...
        print('|___|____________________________|')
        print('| 2 | Initialisation du Boost    |')
        print('|___|____________________________|')
        print('| 3 | MAJ de la base existante   |')
        print('|___|____________________________|')
        ...
        elif _answer == '3' and _no_access == 0:
        ...
        # DEEP LEARNING AVEC SAUVEGARDE DU MODELE
            ...
            FINAL_TRACKER = bt(df_oos,_year_bottom,_year_top,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_invers,_trigger_sl,_verbose,_cash_ini,\
                    _rate,x,_target,_exposure,_size,_sl,_save)
            _save = 0
