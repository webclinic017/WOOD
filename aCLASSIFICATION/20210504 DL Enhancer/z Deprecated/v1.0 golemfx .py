...
print(col.Fore.YELLOW,col.Back.BLUE,'--- Golem FX #v1.00 ---',col.Style.RESET_ALL)
print('')
...
def importohlc(df,df1,_suffix):
    ...
    df['Date'] = pd.to_datetime(df.Date)
    df = df.join(df1[['Open','High','Low','Close']],how='left',on='Date',rsuffix=_suffix)
    try:
        df = df.drop(['Date'+_suffix],axis=1)
    except:
        pass        
    df['Open'+_suffix].fillna(method='ffill', inplace=True)
    df['High'+_suffix].fillna(method='ffill', inplace=True)
    df['Low'+_suffix].fillna(method='ffill', inplace=True)
    df['Close'+_suffix].fillna(method='ffill', inplace=True)
    return(df.sort_index(axis=0))
...
def featuring(df):
    ...
    features['FOPENWEEKLY'] = df['Close'] - df['Open_weekly']#.shift(1)
    features['FHIGHWEEKLY'] = df['Close'] - df['High_weekly']#.shift(1)
    features['FLOWWEEKLY'] = df['Close'] - df['Low_weekly']#.shift(1)
    features['FCLOSEWEEKLY'] = df['Close'] - df['Close_weekly']#.shift(1)
    features['FOPENDAILY'] = df['Close'] - df['Open_daily']#.shift(1)
    features['FHIGHDAILY'] = df['Close'] - df['High_daily']#.shift(1)
    features['FLOWDAILY'] = df['Close'] - df['Low_daily']#.shift(1)
    features['FCLOSEDAILY'] = df['Close'] - df['Close_daily']#.shift(1)
    features['FOPENHOURLY'] = df['Close'] - df['Open_daily']#.shift(1)
    features['FHIGHHOURLY'] = df['Close'] - df['High_daily']#.shift(1)
    features['FLOWHOURLY'] = df['Close'] - df['Low_daily']#.shift(1)
    features['FCLOSEHOURLY'] = df['Close'] - df['Close_daily']#.shift(1)
...
def load_var():
    ...
    _decay = joblib.load('VARS/'+_ticker+'/'+'decay.dag')
    return(_period,_start,_mid,_stop,_last,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_invers,_trigger_sl,_verbose,_cash_ini,_target,_exposure,_sl,df_ratefx,\
        _rate,_size,_trigger_spread,_decay,_period2)


x,_ticker = shuttle()  
_period,_start,_mid,_stop,_last,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_invers,_trigger_sl,_verbose,_cash_ini,_target,_exposure,_sl,df_ratefx\
    ,_rate,_size,_trigger_spread,_decay,_period2 = load_var()
