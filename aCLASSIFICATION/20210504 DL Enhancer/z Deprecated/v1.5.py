...
print(col.Fore.YELLOW,col.Back.BLUE,'--- Golem FX #v1.5 ---',col.Style.RESET_ALL)
print('')
...
def get_daily(df):
    ...
    df1['Open'] = list((df.groupby('Date').OpenBid.first()))
    df1['High'] = list((df.groupby('Date').HighBid.max()))
    df1['Low'] = list((df.groupby('Date').LowBid.min()))
    df1['Close'] = list((df.groupby('Date').CloseBid.last()))
...
def slowstochastic(df,_window=14,_per=3):
...
def rsi(df,_window=2):
    df['MYRSI_'+str(_window)] = talib.RSI(df.Close,_window)
    return(df)
def ADR_talib(df,_window=14):
    df['ADR_talib'] = talib.ATR(df.High,df.Low,df.Close,_window)
    return(df)
...
### STOCHASTIC
def strategy(df,_window):
    ##### CONDITIONS LONG
    _condition_1 = (df['Slow_K'+str(_window)] < 20) & (df['Slow_K'+str(_window)].shift(1) < df['Slow_D'+str(_window)].shift(1)) & (df['Slow_K'+str(_window)] > df['Slow_D'+str(_window)])\
        & (df['Slow_K'+str(_window)].shift(1)< 20) & (df['Slow_K'+str(_window)].shift(2)< 20) & (df['Slow_K'+str(_window)].shift(3)< 20)\
            & (df['Slow_K'+str(_window)].shift(4)< 20)

    ##### CONDITIONS SHORT
    _condition_1_bar = (df['Slow_K'+str(_window)] > 80) & (df['Slow_K'+str(_window)].shift(1) > df['Slow_D'+str(_window)].shift(1)) & (df['Slow_K'+str(_window)] < df['Slow_D'+str(_window)])\
        & (df['Slow_K'+str(_window)].shift(1)>80) & (df['Slow_K'+str(_window)].shift(2)> 80) & (df['Slow_K'+str(_window)].shift(3)> 80)\
            & (df['Slow_K'+str(_window)].shift(4)> 80)
...
def bt(price,_year_bottom,_year_top,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_invers,_trigger_sl,_verbose,_cash_ini,\
    ...
    for i in tqdm(range(0,len(price))):
        ...
        # SI POSITION LONG
            ...
            if  (_trigger_rsi == 1 and price.MYRSI_2[i] < 70):
                _position = 0
                _pnl = (price.CloseBid.iloc[i] - _price_buy_mean) * _size * _open_buy * _rate
                _total += _pnl
                _cash += _pnl
        ...
        # SI POSITION SHORT
            ...
            ### CLOSE SHORT ON RSI DROP OUT
            if  (_trigger_rsi == 1 and price.MYRSI_2[i] < 30):   
                _position = 0
                _pnl = - (price.CloseAsk.iloc[i] - _price_sell_mean) * _size * _open_sell * _rate
                _total += _pnl
                _cash += _pnl
...
# Conexion a FXCM
    ...
    while True:
        ...
        # INITIALISATION DE LA REGRESSION LINEAIRE 
            ...
            print('Désactivée pour l moment... Merci de votre compréhension')
            pass
        ...
        # DEEP LEARNING AVEC SAUVEGARDE DU MODELE
            ...
            while _nb_looser > 0 :
                ...
                ##### Purification of signal by denoising and enhancing
                TRACKER,_nb_looser = bt(df,_year_bottom,_year_top,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_invers,_trigger_sl,_verbose,_cash_ini,\
                        _rate,x,_target,_exposure,_size,_sl,_trigger_rsi)
        ...
        # BACKTEST SUR LE LIVE
            ...
            _verbose = int(input('Verbose : 0 ou 1?'))
            _target = float(input('Entrez la target'))
            _sl = float(input('entrez le s/l'))
