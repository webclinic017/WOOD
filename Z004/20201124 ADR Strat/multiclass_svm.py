...
engine.say("Multi class Support Vector Machine")
engine.runAndWait()

def modelize(df,x,_model):
    
    print(col.Fore.BLUE,'Multi class Support Vector Machine pour',col.Fore.YELLOW,x,col.Style.RESET_ALL)
    print('Modèle :',_model)
    ...
    if counterb[0] > 2 and counterb[1] > 2 and counterb[-1] > 2:
        ...
        # define model
        model = svm.SVC()
        #MLPClassifier(max_iter=300,solver='sgd', activation = 'tanh', alpha=1e-5,hidden_layer_sizes=(150,100,50,3),\
