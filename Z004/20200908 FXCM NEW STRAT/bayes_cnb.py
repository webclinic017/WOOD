...
engine.say("Complement Naive Bayes")
engine.runAndWait()
...
def modelize(x):
    ...
    print(col.Fore.BLUE,'Complement Bayes pour',col.Fore.YELLOW,x,col.Style.RESET_ALL)
    ...
    if counterb[0] > 2 and counterb[1] > 2 and counterv[0] > 2 and counterv[1] > 2:
        ...
        savename = 'JOBLIB/bayes_cnb/Save'+x.replace('/','')+'m5_buy.sav'
        ...
        savename = 'JOBLIB/bayes_cnb/Save'+x.replace('/','')+'m5_sell.sav'
        # serialize model to JOBLIB
        joblib.dump(model, savename)
        print(col.Fore.BLUE,"Joblib Model ",savename," dumped to disk",col.Style.RESET_ALL)
