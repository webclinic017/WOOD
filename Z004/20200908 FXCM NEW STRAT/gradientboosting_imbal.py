...
engine.say("Gradient Boost Classifier Imbalanced")
engine.runAndWait()
...
def modelize(x):
    ...
    print(col.Fore.BLUE,'Gradient Boost Classifier imbalanced pour',col.Fore.YELLOW,x,col.Style.RESET_ALL)
    ...
    if counterb[0] > 2 and counterb[1] > 2 and counterv[0] > 2 and counterv[1] > 2:
        ...
        savename = 'JOBLIB/gradientboosting_imbal/Save'+x.replace('/','')+'m5_buy.sav'
        ...
        savename = 'JOBLIB/gradientboosting_imbal/Save'+x.replace('/','')+'m5_sell.sav'
        # serialize model to JOBLIB
        joblib.dump(model, savename)
        print(col.Fore.BLUE,"Joblib Model ",savename," dumped to disk",col.Style.RESET_ALL)
