...
def get_all_data(TICKER_LIST,_period):
    ...
    delayed_funcs = [delayed(funk)(_ticker=_ticker,_period=_period) for _ticker in (TICKER_LIST)]
...
def get_daily(df_all,TICKER_LIST):
    ...
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(funky)(df_all=df_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes")
...
def get_weekly(daily_all,TICKER_LIST):
    ...
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(funkette)(daily_all=daily_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes")
...
def ema(df_all, _window,TICKER_LIST):
    ...
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(slice_ema)(df_all=df_all,_window=_window,_ticker=_ticker) for _ticker in (TICKER_LIST)]
...
def slowstochastic(df_all,TICKER_LIST,_window=5,_per=3,_plot=0,_ticker=None,start=None,end=None):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(slowstok)(df_all=df_all,_window=_window,_per=_per,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    df_temp = df_temp.append(parallel_pool(delayed_funcs))
...
def faststochastic(df_all,TICKER_LIST,_window=5,_per=3,_plot=0,_ticker=None,start=None,end=None):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(faststok)(df_all=df_all,_window=_window,_per=_per,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    df_temp = df_temp.append(parallel_pool(delayed_funcs))
...
def fullstochastic(df_all,TICKER_LIST,_window=5,_per1=3,_per2=3,_plot=0,_ticker=None,start=None,end=None):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(fullstok)(df_all=df_all,_window=_window,_per1=_per1,_per2=_per2,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    df_temp = df_temp.append(parallel_pool(delayed_funcs))
...
def jyss_oscillator(df_all):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(jyss_osc)(_ticker=_ticker,df_all=df_all) for _ticker in (df_all.Symbol.unique())]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes")
    df_temp = df_temp.append(parallel_pool(delayed_funcs))
...
def rvi(df_all,TICKER_LIST,_window):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(funny)(df_all=df_all,_ticker=_ticker,_window=_window) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes")
    df_temp= df_temp.append(parallel_pool(delayed_funcs))
    return(df_temp.sort_index(axis=0))
...
def onlosma(df_all,TICKER_LIST,_window=8,_plot=0,_ticker=None,start=None,end=None):
    ...
    tt = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(on_lo)(df_all=df_all,_window=_window,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
...
def onhisma(df_all,TICKER_LIST,_window=8,_plot=0,_ticker=None,start=None,end=None):
    ...
    tt = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(on_hi)(df_all=df_all,_window=_window,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    tt = tt.append(parallel_pool(delayed_funcs))
...
def atr(df_all,TICKER_LIST,_window=14,_plot=0,_ticker=None,start=None,end=None):
    ...
    tt = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(get_atr)(df_all=df_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    tt = tt.append(parallel_pool(delayed_funcs))
...
def pivot(weekly_all, TICKER_LIST):
    ...
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(founk)(weekly_all=weekly_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes")
...
def pivotimportdaily(daily_all,weekly_all,TICKER_LIST):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(foolie)(daily_all=daily_all,weekly_all=weekly_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes")
    df_temp = df_temp.append(parallel_pool(delayed_funcs))
    return(df_temp.sort_index(axis=0))
...
def pivotimportdf(df_all,weekly_all,TICKER_LIST):
    ...
    tt = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(get_ppnco)(weekly_all=weekly_all,df_all=df_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    tt = tt.append(parallel_pool(delayed_funcs))
...
def adr(daily_all,_window):
    ...
    temp = pd.DataFrame()
    for _ticker in (daily_all.Symbol.unique()):
...
def getadr(daily_all,df_all, TICKER_LIST):
    ...
    _suffix = '_2Drop'
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(get_ohlc)(df_all=df_all,other_all=daily_all,_ticker=_ticker,_suffix=_suffix) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    tt = tt.append(parallel_pool(delayed_funcs))
...
def adrhnl(daily_all,df_all,TICKER_LIST):
    ...
    df_temp = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(bidiouk)(daily_all=daily_all,df_all=df_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    df_temp = df_temp.append(parallel_pool(delayed_funcs))
...
def sbgamma(df_all,TICKER_LIST):
    ...
    tt = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(sb_gam)(df_all=df_all,_ticker=_ticker) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    tt = tt.append(parallel_pool(delayed_funcs))
    return(tt.sort_index(axis=0))
...
def importohlc(df_all,other_all,TICKER_LIST,_suffix):
    ...
    tt = pd.DataFrame()
    number_of_cpu = joblib.cpu_count()
    delayed_funcs = [delayed(get_ohlc)(df_all=df_all,other_all=other_all,_ticker=_ticker,_suffix=_suffix) for _ticker in (TICKER_LIST)]
    parallel_pool = Parallel(n_jobs=number_of_cpu,prefer="processes") # number_of_cpu
    tt = tt.append(parallel_pool(delayed_funcs))
