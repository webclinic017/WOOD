...
def bt(price,_year_bottom,_year_top,_nb_bougie_exit,_trigger_reengage,_trigger_target,_trigger_exposure,_verbose,_cash_ini,_rate,x,_confidence) :
    engine = pyttsx3.init()
    ...
    for i in range(0,len(price)):
        ...
        if _position == 0:
            # BUY SIGNAL
            if  abs(price.ADRLow[i] - price.Low[i])/100 < _confidence: #and price.Low_Ask[i] >= price.ADRMin[i]: 
                _pnl = 0
                _open_buy += 1
                _equity = _cash + _pnl
            ...
            # SELL SIGNAL
            elif abs(price.ADRHigh[i] - price.High[i])/100 < _confidence: #and price.High_Bid[i] < price.ADRMax[i] :  
                _pnl = 0
                _open_sell += 1
                _equity = _cash + _pnl
                EQUITY.append(_equity)
            ...
            if abs(price.ADRLow[i] - price.Low[i])/100 < _confidence and i - _index_entry < _nb_bougie_exit and _trigger_reengage == 1 and _open_buy < _trigger_exposure :
                _pnl = 0
                _open_buy += 1
                _equity = _cash + _pnl
                EQUITY.append(_equity)
