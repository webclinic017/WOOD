...
engine.say("M L P Classifier")
engine.runAndWait()

def modelize(df,x,_model):
    
    print(col.Fore.BLUE,'MLPClassifier pour',col.Fore.YELLOW,x,col.Style.RESET_ALL)
    ...
    if counterb[-1] > 2 and counterb[1] > 2:
        ...
        # define model
        model = MLPClassifier(solver='adam', activation = 'tanh', alpha=1e-5,hidden_layer_sizes=(5, 2), random_state=1) # lbfgs
        # fit model
        ...
        precb = round(precision_score(testyb, yhatb,labels=[-1,1], average='micro') * 100,2) # labels=[-1,1], average='micro' 'weighted' 'macro' 'None'
        recallb = round(recall_score(testyb, yhatb,labels=[-1,1], average='micro') * 100,2) # labels=[-1,1], average='micro'  'weighted' 'macro' 'None'
        f1b = round(f1_score(testyb, yhatb,labels=[-1,1], average='micro') * 100,2) # labels=[-1,1], average='micro'   'weighted' 'macro''None'
        ...
        '''#precisionb, recallb, thresholdsb = precision_recall_curve(testyb, yhatb,'micro')
        
        # plot the roc curve for the model
        no_skillb= len(testyb[testyb==1]) / len(testyb)

        # convert to f score
        fscoreb = (2 * precb * recallb) / (precb + recallb)
        ...
        plt.plot(recallb, precb, marker='.', label='Logistic')
        plt.scatter(recallb[ixb], precb[ixb], marker='o', color='black', label='Best')
        # axis labels
        plt.xlabel('Recall')
