...
engine.say("Ada Boost Classifier")
engine.runAndWait()
...
def modelize(df,x,_model,_verbose=0):
    ...
    print(col.Fore.BLUE,'AdaBoost Classifier pour',col.Fore.YELLOW,x,col.Style.RESET_ALL)
    ...
    if counterb[1] > 2 and counterb[0] > 2:
        ...
        model = AdaBoostClassifier(algorithm='SAMME.R', base_estimator=LogisticRegression(C=6.091,
                                                        ...
                                                        verbose=0,
                                                        warm_start=False),
                                                        cv=StratifiedKFold(n_splits=10, random_state=2711, shuffle=False),
                                                        method='sigmoid'),learning_rate=1.0, n_estimators=100, random_state=2711)
        # fit model
        ...
        savename = 'JOBLIB/adaboost/Save'+x.replace('/','')+'m5.sav'
