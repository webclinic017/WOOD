...
def modelize(df,x,_model,_verb,_C, _kernel,_degree, _gamma, _coef0, _shrinking, _probability, _tol, _cache_size,\
 _class_weight, _verbose, _max_iter, _decision_function_shape, _break_ties, _random_state):
    ...
    if counterb[1] > 2 and counterb[0] > 2:
        ...
        # define model
        model = svm.SVC(_C, _kernel,_degree, _gamma, _coef0, _shrinking, _probability, _tol, _cache_size, _class_weight, _verbose\
        , _max_iter, _decision_function_shape, _break_ties, _random_state)
        # fit model
    ...
    if _kernel == 'linear':
        ...
        return('Pas de coeficient => juste pour le kernel lineaire')
