...
engine.say("Bagging K Neigbhors Classifier")
engine.runAndWait()

def modelize(df,x,_model,_verbose=0):
    
    print(col.Fore.BLUE,'Bagging K Neigbhors Classifier pour',col.Fore.YELLOW,x,col.Style.RESET_ALL)
    ...
    if counterb[1] > 2 and counterb[0] > 2:
        ...
        # define model
        model = BaggingClassifier(KNeighborsClassifier(),n_estimators=40,max_samples=0.5, max_features=0.5,\
                            bootstrap_features=True, oob_score=True, warm_start=False, n_jobs=-1, random_state=True, verbose=0)
        # fit model
